...
import { CompanyScreen as LiveCompanyScreen } from '../screens/CompanyScreen'
import { AwaitingScreen as LiveAwaitingScreen } from '../screens/AwaitingScreen'
...
type Screen =
  ...
  | { kind: 'equipment-list'; locationId: string; locationName?: string }
  | { kind: 'equipment-item'; equipment: api.Equipment }
  | { kind: 'chat-detail'; ticketId: string; ticketTitle: string }
  | { kind: 'planning' }
  | { kind: 'add-equipment' }
...
// ─── Live data layer ───────────────────────────────────────────────────────────
// Живой слой ПОВЕРХ моков. demo=true → источник данных OBJECTS (моки, без сети).
// demo=false → данные из api.board() через boardToObjects. Оба пути параллельны.
type LiveCtx = {
  demo: boolean
  meId: string | null
  meName: string | null
...
const LiveContext = createContext<LiveCtx>({
  demo: true, meId: null, meName: null, meInitials: null, objects: OBJECTS,
  boardLoading: false, boardError: null, reloadBoard: async () => {},
})
const useLive = () => useContext(LiveContext)

// Имя + инициалы пользователя из /auth/me (login возвращает user: Me)
function userDisplay(u: { firstName?: string | null; lastName?: string | null; email?: string } | null | undefined): { name: string; initials: string } {
  if (!u) return { name: 'Пользователь', initials: '—' }
  const name = [u.firstName, u.lastName].filter(Boolean).join(' ').trim() || u.email || 'Пользователь'
...
// ─── Object Card ──────────────────────────────────────────────────────────────
function ObjectCard({obj,onOpenTicket,searchQuery='',onOpenEquipment,demo=true,addToast}:{obj:ServiceObject;onOpenTicket:(t:Ticket)=>void;searchQuery?:string;onOpenEquipment?:(locationId:string,locationName:string)=>void;demo?:boolean;addToast?:(t:string,tp?:ToastMsg['type'])=>void}) {
  ...
  const [actsPeriod,setActsPeriod]=useState<'7d'|'30d'|'90d'>('30d')
  // Акты: API нет → берём DONE-заявки за период (// TODO: /acts endpoint отсутствует)
  const [actsTickets,setActsTickets]=useState<api.TicketListItem[]>([])
  const [actsLoading,setActsLoading]=useState(false)
  useEffect(()=>{
    if(demo||!actsOpen)return
    let alive=true
    setActsLoading(true)
    api.getTicketsByLocation(obj.id,'DONE').then(list=>{
      if(!alive)return
      const days={'7d':7,'30d':30,'90d':90}[actsPeriod]
      const since=Date.now()-days*86400000
      setActsTickets(list.filter(t=>new Date(t.statusUpdatedAt||t.createdAt).getTime()>=since))
    }).catch(()=>{if(alive)setActsTickets([])}).finally(()=>{if(alive)setActsLoading(false)})
    return ()=>{alive=false}
  },[demo,actsOpen,actsPeriod,obj.id])
  return (
    <div className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${obj.counts.overdue>0?'border-red-200':'border-slate-100'}`}>
      ...
      {expanded&&(
        <div className="border-t border-slate-50">
            ...
            <div className="bg-blue-50 border-b border-blue-100 px-4 py-3">
              ...
              <div className="bg-white rounded-xl p-2.5 mb-2 border border-blue-200">
                {demo?(<>
                  ...
                  <p className="text-[10px] text-slate-400">Загрузка…</p>
                ):(<>
                  <p className="text-[10px] text-slate-500">Завершённых работ за период: <strong>{actsTickets.length} {actsTickets.length===1?'заявка':actsTickets.length>=2&&actsTickets.length<=4?'заявки':'заявок'}</strong></p>
                  <p className="text-[10px] text-slate-400 mt-0.5 line-clamp-2">{actsTickets.length?actsTickets.slice(0,5).map(t=>t.problemText||`#${t.ticketNumber}`).join(' · '):'Нет завершённых работ за период'}</p>
                </>)}
              </div>
              {/* TODO: /acts endpoint нет в API — PDF не генерируется, используем DONE-заявки */}
              <button onClick={e=>{e.stopPropagation();if(demo){addToast?.('Акт сформирован (демо)','success')}else{addToast?.('Формирование акта PDF — нет в API','info')}}} className="w-full py-2 bg-blue-600 text-white rounded-xl text-[12px] font-bold">📄 Сформировать акт PDF</button>
            </div>
          )}
          ...
          {/* Оборудование (живой режим: onOpenEquipment передаётся только при !demo) */}
          {onOpenEquipment&&<button onClick={e=>{e.stopPropagation();onOpenEquipment(obj.id,obj.name)}} className="w-full flex items-center gap-2 px-4 py-2.5 border-t border-slate-50 text-[11px] font-bold text-blue-600 active:bg-blue-50 transition-colors">
            <IconTool size={14}/>Оборудование →
          </button>}
...
function HomeScreen({onOpenTicket,role,push,addToast}:{onOpenTicket:(t:Ticket)=>void;role:UserRole;push:(s:Screen)=>void;addToast:(t:string,tp?:ToastMsg['type'])=>void}) {
  ...
  // Источник данных: демо → OBJECTS (моки), реальный вход → доска из api.board()
  const {objects:SRC,boardLoading,boardError,demo,reloadBoard,meName,meInitials}=useLive()
  // Имя+аватар: demo → статика Figma; live → из /auth/me (через login user)
  const displayName=demo?'Дмитрий Ковалёв':(meName||'Пользователь')
  const displayInitials=demo?'ДК':(meInitials||'—')
  const allT=SRC.flatMap(o=>o.tickets)

  const hq=homeSearch.trim().toLowerCase()
  const awaitingCount=allT.filter(t=>t.status==='AWAITING_ACCEPTANCE').length
  const myActionCount=allT.filter(t=>filterTickets([t],'myaction',role).length>0).length
  const totalNew=SRC.reduce((s,o)=>s+o.counts.newCount,0)
  const totalOverdue=SRC.reduce((s,o)=>s+o.counts.overdue,0)
  const totalInWork=SRC.reduce((s,o)=>s+o.counts.inWork,0)
    ...
    <div className="flex-1 overflow-hidden flex flex-col">
      ...
      <div className="bg-white px-4 pt-2 pb-3 border-b border-slate-100 flex-shrink-0">
        <div className="flex items-center justify-between mb-1">
          <div><p className="text-[11px] text-slate-400">Управление объектами</p><h1 className="text-[18px] font-bold text-slate-900">{displayName}</h1></div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={()=>push({kind:'profile'})} className="w-9 h-9 rounded-full bg-blue-600 flex items-center justify-center shadow-sm"><span className="text-white text-[12px] font-bold">{displayInitials}</span></button>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50" style={{scrollbarWidth:'none'}}>
        <div className="px-4 pt-3 pb-2">
          <div className="grid grid-cols-4 gap-2 mb-2">
            {[{l:'Объектов',v:SRC.length,col:'text-blue-700',bg:'bg-blue-50 border-blue-100',f:'all' as StatusFilter},
        ...
        <div className="flex items-center gap-2 px-4 pb-3">
          <p className="text-[12px] font-semibold text-slate-500 flex-1">{filtered.length===SRC.length?`${SRC.length} объектов`:`${filtered.length} из ${SRC.length}`}</p>
        ...
        <div className="px-4 pb-5 flex flex-col gap-3 pt-2">
          ...
          <div className="flex items-center justify-between -mt-1">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${demo?'bg-slate-100 text-slate-500':'bg-emerald-50 text-emerald-700'}`}>{demo?'Демо-режим':'Живые данные'}</span>
            {!demo&&<button onClick={()=>reloadBoard()} disabled={boardLoading} className="text-[11px] font-semibold text-blue-600 active:text-blue-700">{boardLoading?'Обновление…':'Обновить'}</button>}
          </div>
          {/* Error (живой режим) */}
          {!demo&&boardError&&(
            <div className="flex flex-col items-center gap-3 py-10">
              <span className="text-[36px]">⚠️</span>
              <p className="text-[13px] text-slate-600 text-center px-6">{boardError}</p>
              <button onClick={()=>reloadBoard()} className="text-[12px] font-bold text-white bg-blue-600 px-4 py-2 rounded-xl active:bg-blue-700">Повторить</button>
            </div>
          )}
          {/* Loading (живой режим, данных ещё нет) */}
          {!demo&&!boardError&&boardLoading&&SRC.length===0&&(
            <div className="flex flex-col items-center gap-3 py-12"><span className="text-[28px] animate-pulse">⏳</span><p className="text-[13px] text-slate-500">Загрузка заявок…</p></div>
          )}
          {/* Empty (живой режим, заявок нет) */}
          {!demo&&!boardError&&!boardLoading&&SRC.length===0&&(
            <div className="flex flex-col items-center gap-3 py-12"><span className="text-[40px]">📭</span><p className="text-[13px] text-slate-500">Заявок пока нет</p></div>
          )}
          {(demo||!boardError)&&filtered.map(o=><ObjectCard key={o.id} obj={o} onOpenTicket={onOpenTicket} searchQuery={hq} demo={demo} addToast={addToast} onOpenEquipment={demo?undefined:(id,name)=>push({kind:'equipment-list',locationId:id,locationName:name})}/>)}
          {(demo||!boardError)&&SRC.length>0&&filtered.length===0&&<div className="flex flex-col items-center gap-3 py-12"><span className="text-[40px]">{hq?'🔍':'🏢'}</span><p className="text-[13px] text-slate-500">{hq?'Ничего не найдено':'Нет объектов по фильтру'}</p><button onClick={()=>{setFilters(DEFAULT_FILTERS);setHomeSearch('')}} className="text-[12px] font-semibold text-blue-600">Сбросить</button></div>}
          {!hq&&filtered.length>0&&<p className="text-center text-[10px] text-slate-400 mt-1">Нажмите на объект, чтобы раскрыть заявки</p>}
        </div>
      </div>
    </div>
  )
...
// ─── Add Receipt Modal ────────────────────────────────────────────────────────
// ─── Обходы (живой режим, inspection API) ───────────────────────────────────────
const RUN_ITEM_STATUS:{k:api.InspectionRunItemStatus;label:string;cls:string}[]=[
  {k:'OK',label:'Ок',cls:'bg-emerald-500 text-white'},
  {k:'ISSUE',label:'Замечание',cls:'bg-amber-500 text-white'},
  {k:'CRITICAL',label:'Критично',cls:'bg-red-500 text-white'},
  {k:'SKIPPED',label:'Пропуск',cls:'bg-slate-400 text-white'},
]
function itemStatusBadge(s:api.InspectionRunItemStatus){return ({PENDING:'bg-slate-100 text-slate-400',OK:'bg-emerald-100 text-emerald-700 font-bold',ISSUE:'bg-amber-100 text-amber-700 font-bold',CRITICAL:'bg-red-100 text-red-700 font-bold',SKIPPED:'bg-slate-100 text-slate-400'} as Record<string,string>)[s]}

function LivePatrolsScreen({addToast}:{addToast:(t:string,tp?:ToastMsg['type'])=>void}) {
  type LTab='runs'|'templates'|'analytics'
  const [tab,setTab]=useState<LTab>('runs')
  const [runs,setRuns]=useState<api.InspectionRunListItem[]>([])
  const [templates,setTemplates]=useState<api.InspectionTemplate[]>([])
  const [loading,setLoading]=useState(false)
  const [error,setError]=useState<string|null>(null)
  const [detail,setDetail]=useState<api.InspectionRun|null>(null)
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const [r,t]=await Promise.all([api.inspectionRuns(),api.inspectionTemplates()])
      setRuns(Array.isArray(r)?r:[]);setTemplates(Array.isArray(t)?t:[])
      try{ const cs=await api.problemCategories(); setCatId(cs?.[0]?.id) }catch{/* категория для create-ticket опц. */}
    }catch(e:any){ setError(e?.message||'Не удалось загрузить обходы') }
    finally{ setLoading(false) }
  },[])
  useEffect(()=>{ load() },[load])

  async function openRun(id:string){ setBusy('open');try{ setDetail(await api.inspectionRun(id)) }catch(e:any){ addToast(e?.message||'Не удалось открыть обход','error') }finally{ setBusy(null) } }
  async function refreshDetail(){ if(detail) try{ setDetail(await api.inspectionRun(detail.id)) }catch{/* noop */} }

  async function setItemStatus(item:api.InspectionRunItem,status:api.InspectionRunItemStatus){
    if(!detail)return
    setBusy(item.id)
    try{ await api.updateRunItem(detail.id,item.id,{status}); await refreshDetail() }
    catch(e:any){ addToast(e?.message||'Ошибка','error') }
    finally{ setBusy(null) }
  }
  async function makeTicket(item:api.InspectionRunItem){
    if(!detail||!catId){ addToast('Нет категории для заявки','error'); return }
    setBusy('tkt-'+item.id)
    try{ await api.createTicketFromItem(detail.id,item.id,{categoryId:catId,title:item.title}); addToast('Заявка создана из пункта','success'); await refreshDetail() }
    catch(e:any){ addToast(e?.message||'Не удалось создать заявку','error') }
    finally{ setBusy(null) }
  }
  async function onPickPhoto(e:any){
    const file:File|undefined=e?.target?.files?.[0]; if(e?.target)e.target.value=''
    const itemId=photoTarget.current; if(!file||!detail||!itemId)return
    if(file.size>api.INSPECTION_ATTACHMENT_MAX_BYTES){ addToast('Файл больше 10 МБ','error'); return }
    setBusy('photo-'+itemId)
    try{ await api.uploadRunItemPhoto(detail.id,itemId,file); addToast('Фото добавлено','success'); await refreshDetail() }
    catch(e:any){ addToast(e?.message||'Не удалось загрузить фото','error') }
    finally{ setBusy(null) }
  }
  async function finish(){
    if(!detail)return
    const pending=detail.items.filter(i=>i.status==='PENDING'&&i.isRequired).length
    if(pending>0){ addToast(`Остались обязательные пункты: ${pending}`,'error'); return }
    setBusy('finish')
    try{ await api.completeRun(detail.id); addToast('Обход завершён','success'); setDetail(null); await load() }
    catch(e:any){ addToast(e?.message||'Не удалось завершить','error') }
    finally{ setBusy(null) }
  }

  // ── Детальный чек-лист ──
  if(detail){
    const done=detail.items.filter(i=>i.status!=='PENDING').length
    return (
      <div className="flex-1 overflow-hidden flex flex-col">
        <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={onPickPhoto}/>
        <BackHeader title={detail.title} subtitle={`${detail.location.name} · ${done}/${detail.items.length}`} onBack={()=>setDetail(null)}/>
        <div className="flex-1 overflow-y-auto px-4 py-4 bg-slate-50 flex flex-col gap-3" style={{scrollbarWidth:'none'}}>
          {detail.items.slice().sort((a,b)=>a.sortOrder-b.sortOrder).map(item=>(
            <div key={item.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3.5">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex-1"><p className="text-[13px] font-bold text-slate-800">{item.title}{item.isRequired&&<span className="text-red-500"> *</span>}</p>{item.description&&<p className="text-[11px] text-slate-400 mt-0.5">{item.description}</p>}</div>
                <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${itemStatusBadge(item.status)}`}>{item.status}</span>
              </div>
              <div className="flex gap-1.5 flex-wrap">
                {RUN_ITEM_STATUS.map(s=><button key={s.k} disabled={busy===item.id} onClick={()=>setItemStatus(item,s.k)} className={`text-[11px] font-semibold px-2.5 py-1.5 rounded-lg transition-all disabled:opacity-50 ${item.status===s.k?s.cls:'bg-slate-100 text-slate-500'}`}>{s.label}</button>)}
              </div>
              {(item.status==='ISSUE'||item.status==='CRITICAL')&&(
                <div className="mt-2.5 flex items-center gap-2">
                  <button disabled={busy==='photo-'+item.id} onClick={()=>{photoTarget.current=item.id;fileRef.current?.click()}} className="flex items-center gap-1 text-[11px] font-semibold text-blue-600 disabled:opacity-50"><IconCamera size={14}/>Фото{item.attachments.length>0?` (${item.attachments.length})`:''}</button>
                  {item.ticketId
                    ? <span className="text-[11px] font-semibold text-emerald-600 ml-auto">Заявка создана ✓</span>
                    : <button disabled={busy==='tkt-'+item.id} onClick={()=>makeTicket(item)} className="text-[11px] font-semibold text-red-600 ml-auto disabled:opacity-50">Создать заявку →</button>}
                </div>
              )}
              {item.attachments.length>0&&<div className="mt-2 flex gap-2 flex-wrap">{item.attachments.map(a=><img key={a.id} src={api.attachmentSrc(a.url)} className="w-14 h-14 rounded-lg object-cover border border-slate-200"/>)}</div>}
            </div>
          ))}
          {detail.status==='IN_PROGRESS'
            ? <button disabled={busy==='finish'} onClick={finish} className="w-full py-4 bg-blue-600 text-white rounded-[12px] text-[15px] font-bold shadow-lg shadow-blue-200 disabled:opacity-60 mt-1">{busy==='finish'?'Завершение…':'Завершить обход'}</button>
            : <div className="bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-4 text-center text-[13px] font-bold text-emerald-700 mt-1">Обход завершён ✓</div>}
          <div className="h-4"/>
        </div>
      </div>
    )
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      <div className="bg-white px-4 pt-3 pb-2 border-b border-slate-100 flex-shrink-0">
        <h1 className="text-[18px] font-bold text-slate-900 mb-2">Обходы</h1>
        <div className="flex bg-slate-100 rounded-xl p-1">
          {([['runs','Обходы'],['templates','Шаблоны'],['analytics','Аналитика']] as const).map(([k,l])=>(
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 bg-slate-50" style={{scrollbarWidth:'none'}}>
        ...
        {!loading&&!error&&tab==='runs'&&<div className="flex flex-col gap-4">
          {[['Активные',active],['История',completed]].map(([label,list])=>(
            <div key={label as string}>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wide mb-2">{label as string} ({(list as any[]).length})</p>
              <div className="flex flex-col gap-2">
                {(list as api.InspectionRunListItem[]).map(r=>(
                  <button key={r.id} onClick={()=>openRun(r.id)} disabled={busy==='open'} className={`bg-white rounded-2xl border border-slate-200 border-l-4 shadow-sm p-3.5 text-left active:scale-[0.99] transition-transform disabled:opacity-60 ${r.status==='IN_PROGRESS'?'border-l-blue-500':'border-l-emerald-500'}`}>
                    <div className="flex items-center justify-between mb-1"><p className="text-[13px] font-bold text-slate-800">{r.title}</p><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${r.status==='IN_PROGRESS'?'bg-blue-100 text-blue-700':'bg-emerald-100 text-emerald-700'}`}>{r.status==='IN_PROGRESS'?'В процессе':'Завершён'}</span></div>
                    <p className="text-[11px] text-slate-400">{r.template.name} · {r.location.name} · {r._count.items} пунктов</p>
                  </button>
                ))}
                {(list as any[]).length===0&&<p className="text-[12px] text-slate-400 py-2">Нет обходов</p>}
              </div>
            </div>
          ))}
        </div>}
        {!loading&&!error&&tab==='templates'&&<div className="flex flex-col gap-2">
          {templates.map(t=>(
            <div key={t.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3.5">
              <div className="flex items-center justify-between"><p className="text-[13px] font-bold text-slate-800">{t.name}</p><span className="text-[10px] font-bold text-slate-400">{t.items.length} пунктов</span></div>
              {t.description&&<p className="text-[11px] text-slate-400 mt-0.5">{t.description}</p>}
            </div>
          ))}
          {templates.length===0&&<p className="text-[12px] text-slate-400 py-2">Нет шаблонов</p>}
        </div>}
        {!loading&&!error&&tab==='analytics'&&<div className="grid grid-cols-2 gap-2.5">
          {[['Всего обходов',runs.length,'text-blue-700 bg-blue-50 border-blue-100'],['Активные',active.length,'text-violet-700 bg-violet-50 border-violet-100'],['Завершено',completed.length,'text-emerald-700 bg-emerald-50 border-emerald-100'],['Пунктов всего',runs.reduce((s,r)=>s+r._count.items,0),'text-slate-700 bg-slate-50 border-slate-200']].map(([l,v,cls])=>(
            <div key={l as string} className={`rounded-2xl border p-3.5 ${cls}`}><p className="text-[22px] font-bold leading-none mb-1">{v as number}</p><p className="text-[11px] text-slate-500">{l as string}</p></div>
          ))}
          {import.meta.env.DEV&&<p className="col-span-2 text-[11px] text-slate-400 mt-1">Считается из GET /inspection/runs. Детальная статистика по пунктам (замечания/критичные) — TODO: требует обхода детали каждого run.</p>}
        </div>}
      </div>
    </div>
  )
...
function CreateTicketScreen({onBack,addToast}:{onBack:()=>void;addToast:(t:string,tp?:ToastMsg['type'])=>void}) {
  const {demo,reloadBoard}=useLive()
  const [mode,setMode]=useState<'quick'|'full'>('quick')
  const [objectId,setObjectId]=useState('')   // live: locationId; demo: OBJECTS.id
  const [category,setCategory]=useState('')    // live: problemCategoryId; demo: имя категории
  const [description,setDescription]=useState('')
  const [hasPhoto,setHasPhoto]=useState(false) // демо-мок фото
  ...
  // Живые справочники
  const [locs,setLocs]=useState<api.LocationItem[]>([])
  const [cats,setCats]=useState<api.ProblemCategoryItem[]>([])
  const [refsLoading,setRefsLoading]=useState(false)
  const [refsError,setRefsError]=useState<string|null>(null)
  const [scopeCompanyId,setScopeCompanyId]=useState<string|undefined>(undefined)
  // Живые фото (черновые вложения до создания)
  ...
  const loadRefs=useCallback(async()=>{
    ...
    try{
      ...
      let ls=await api.locations()
      if(ls.length===0){
        // провайдер-техник: локации/категории живут в linked-client компании
        try{ const bc=await api.technicianBoundContexts(); scope=bc?.[0]?.clientCompany?.id; if(scope) ls=await api.locations(scope) }catch{/* остаётся пусто */}
      }
      const cs=await api.problemCategories(scope)
      // защита: всегда массив (если эндпоинт вернул не то — не роняем экран)
      setScopeCompanyId(scope);setLocs(Array.isArray(ls)?ls:[]);setCats(Array.isArray(cs)?cs:[])
    }catch(e:any){ setRefsError(e?.message||'Не удалось загрузить справочники') }
    finally{ setRefsLoading(false) }
  },[])
  useEffect(()=>{ if(!demo) loadRefs() },[demo,loadRefs])

  async function pickPhoto(e:any){
    const file:File|undefined=e?.target?.files?.[0]
    if(e?.target) e.target.value=''
    if(!file)return
    if(photos.length>=3){ addToast('Максимум 3 фото','error'); return }
    ...
    setPhotoBusy(true)
    try{ const a=await api.uploadDraftAttachment(file); setPhotos(p=>[...p,{id:a.id,url:a.url}]) }
    catch(err:any){ addToast(err?.message||'Не удалось загрузить фото','error') }
    finally{ setPhotoBusy(false) }
  }

  const validate=()=>{const e:Record<string,string>={};if(!objectId)e.object='Выберите объект';if(!category)e.category='Выберите категорию';if(description.trim().length<5)e.description='Опишите проблему (мин. 5 символов)';if(mode==='full'&&isUrgent&&!urgencyReason.trim())e.urgency='Укажите причину срочности';setErrors(e);return!Object.keys(e).length}

  async function submit(){
    if(!validate())return
    if(demo){ addToast('Заявка создана (демо)','success'); onBack(); return }
    setSubmitting(true)
    try{
      const dto:api.CreateTicketInput={
        locationId:objectId,
        problemCategoryId:category||undefined,
        problemText:description.trim(),
        urgency: (mode==='full'&&isUrgent)?'URGENT':'NOT_URGENT',
        createMode:mode,
        ...(mode==='full'?{
          requesterName:requesterName.trim()||undefined,
          requesterPhone:requesterPhone.trim()||undefined,
          attachmentIds:photos.length?photos.map(p=>p.id):undefined,
        }:{}),
        ...(scopeCompanyId?{clientCompanyId:scopeCompanyId}:{}),
      }
      const res=await api.createTicket(dto)
      addToast(`Заявка #${res.ticket?.ticketNumber??''} создана`,'success')
      reloadBoard()
      onBack()
    }catch(e:any){ addToast(e?.message||'Не удалось создать заявку','error') }
    ...
    <div className="flex-1 overflow-hidden flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4" style={{scrollbarWidth:'none'}}>
        ...
        <div className="flex mb-4 bg-slate-100 rounded-xl p-1">
          {([['quick','Быстро'],['full','Полная форма']] as const).map(([m,l])=>(
            <button key={m} onClick={()=>setMode(m)} className={`flex-1 py-2 text-[12px] transition-all ${mode===m?'bg-white rounded-lg shadow-sm font-semibold text-blue-600':'text-slate-400'}`}>{l}</button>
          ))}
        </div>
        {!demo&&<div className="mb-3 flex items-center justify-between"><span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700">Живые данные</span>{refsLoading&&<span className="text-[11px] text-slate-400">Загрузка справочников…</span>}{refsError&&<button onClick={()=>loadRefs()} className="text-[11px] font-semibold text-blue-600">Повторить</button>}</div>}
        {refsError&&!demo&&<div className="mb-4 bg-red-50 border border-red-200 rounded-xl px-3 py-2.5 text-[12px] text-red-700">{refsError}</div>}
        <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" className="hidden" onChange={pickPhoto}/>
        <div className="mb-4"><label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Объект *</label><div className="relative"><select value={objectId} onChange={e=>{setObjectId(e.target.value);setErrors(p=>({...p,object:''}))}} disabled={!demo&&refsLoading} className="appearance-none w-full bg-slate-100 rounded-xl px-4 py-3 pr-10 text-[13px] text-slate-800 outline-none focus:ring-2 focus:ring-blue-300 disabled:opacity-60"><option value="">Выберите объект...</option>{demo?OBJECTS.map(o=><option key={o.id} value={o.id}>{o.icon} {o.name}</option>):locs.map(l=><option key={l.id} value={l.id}>{l.name}{l.city?` · ${l.city}`:''}</option>)}</select><IconChevronDown size={16} className="absolute right-3.5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400"/></div>{errors.object&&<p className="text-[11px] text-red-600 mt-1">{errors.object}</p>}</div>
        <div className="mb-4"><label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Категория *</label><div className="relative"><select value={category} onChange={e=>{setCategory(e.target.value);setErrors(p=>({...p,category:''}))}} disabled={!demo&&refsLoading} className="appearance-none w-full bg-slate-100 rounded-xl px-4 py-3 pr-10 text-[13px] text-slate-800 outline-none focus:ring-2 focus:ring-blue-300 disabled:opacity-60"><option value="">Выберите категорию...</option>{demo?CATEGORIES.map(c=><option key={c}>{c}</option>):cats.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><IconChevronDown size={16} className="absolute right-3.5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400"/></div>{errors.category&&<p className="text-[11px] text-red-600 mt-1">{errors.category}</p>}</div>
        <div className="mb-4"><label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Описание *</label><textarea rows={4} value={description} onChange={e=>{setDescription(e.target.value);setErrors(p=>({...p,description:''}))}} placeholder="Опишите проблему подробно..." className="w-full bg-slate-100 rounded-xl px-4 py-3 text-[13px] text-slate-800 outline-none focus:ring-2 focus:ring-blue-300 resize-none"/>{errors.description&&<p className="text-[11px] text-red-600 mt-1">{errors.description}</p>}</div>
        {/* Полная форма: срочность + фото + заявитель */}
        {mode==='full'&&<>
        {!demo&&<div className="mb-4">
          <label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Фото (до 3)</label>
          <div className="flex gap-2 flex-wrap">
            {photos.map(p=><img key={p.id} src={api.attachmentSrc(p.url)} className="w-20 h-20 rounded-xl object-cover border border-slate-200 bg-slate-50"/>)}
            {photos.length<3&&<button disabled={photoBusy} onClick={()=>fileRef.current?.click()} className="w-20 h-20 rounded-xl border-2 border-dashed border-slate-300 text-slate-400 flex items-center justify-center disabled:opacity-50">{photoBusy?<span className="text-[11px] font-semibold">…</span>:<IconCamera size={24}/>}</button>}
          </div>
        </div>}
        {demo&&<div className="mb-4"><label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Фото</label><div className="flex gap-2">{!hasPhoto?<><button onClick={()=>setHasPhoto(true)} className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-50 border border-slate-200 rounded-xl text-[12px] font-semibold text-slate-600">📷 Сделать фото</button><button onClick={()=>setHasPhoto(true)} className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-50 border border-slate-200 rounded-xl text-[12px] font-semibold text-slate-600">🖼️ Из галереи</button></>:<div className="w-full bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-3 flex items-center gap-2"><span className="text-emerald-600">✓</span><span className="text-[12px] font-semibold text-emerald-700">Фото добавлено</span><button onClick={()=>setHasPhoto(false)} className="ml-auto text-[10px] text-slate-400">Удалить</button></div>}</div></div>}
        <div className="mb-4">
          <label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Заявитель</label>
          <input value={requesterName} onChange={e=>setRequesterName(e.target.value)} placeholder="Имя" className="mt-1.5 w-full bg-slate-100 rounded-xl px-4 py-3 text-[13px] text-slate-800 outline-none focus:ring-2 focus:ring-blue-300"/>
          <input value={requesterPhone} onChange={e=>setRequesterPhone(e.target.value)} inputMode="tel" placeholder="Телефон" className="mt-2 w-full bg-slate-100 rounded-xl px-4 py-3 text-[13px] text-slate-800 outline-none focus:ring-2 focus:ring-blue-300"/>
        </div>
        <div className="mb-4">
          <label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Срочность</label>
          {/* Urgency modal */}
          {urgencyModal&&(
            <div className="absolute inset-0 z-50 bg-black/50 flex items-center justify-center px-6">
              <div className="bg-white rounded-3xl shadow-2xl p-6 w-full">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={()=>{setUrgencyModal(false);setIsUrgent(true)}} className="flex-1 py-3 bg-blue-600 text-white rounded-2xl text-[13px] font-bold shadow-md shadow-blue-200 active:scale-95 transition-transform">Понятно</button>
                </div>
              </div>
            </div>
          )}
          <div className="mt-1.5 flex bg-slate-100 rounded-xl p-1">
            <button onClick={()=>{setIsUrgent(false);setUrgencyReason('')}} className={`flex-1 py-2 text-[13px] transition-all ${!isUrgent?'bg-white rounded-lg shadow-sm font-semibold text-blue-600':'text-slate-400'}`}>Обычная</button>
            <button onClick={()=>setUrgencyModal(true)} className={`flex-1 py-2 text-[13px] flex items-center justify-center gap-1 transition-all ${isUrgent?'bg-white rounded-lg shadow-sm font-semibold text-red-600':'text-slate-400'}`}><IconBolt size={15}/>Срочная</button>
          </div>
          {isUrgent&&<div className="mt-3"><label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Причина срочности *</label><textarea rows={2} value={urgencyReason} onChange={e=>{setUrgencyReason(e.target.value);setErrors(p=>({...p,urgency:''}))}} placeholder="Почему срочно? Какой конкретно риск?" className="mt-1.5 w-full bg-white border border-red-200 rounded-xl px-3 py-2.5 text-[13px] text-slate-800 outline-none focus:border-red-400 resize-none"/>{errors.urgency&&<p className="text-[11px] text-red-600 mt-1">{errors.urgency}</p>}{/* urgencyReason — клиентская подсказка; в API поля нет, не отправляется */}</div>}
        </div>
        </>}
        <button onClick={submit} disabled={submitting||(!demo&&refsLoading)} className="w-full py-4 bg-blue-600 text-white rounded-[12px] text-[15px] font-bold shadow-lg shadow-blue-200 disabled:opacity-60">{submitting?'Создание…':'Создать заявку'}</button>
        <div className="h-6"/>
      </div>
    </div>
  )
...
function ActionsTab({ticket,role,addToast,onViewPhotos}:{ticket:Ticket;role:UserRole;addToast:(t:string,tp?:ToastMsg['type'])=>void;onViewPhotos?:()=>void}) {
  ...
  // Живой статус заявки (демо → берём из снимка ticket). Меняется после успешных действий.
  const [liveStatus,setLiveStatus]=useState<TicketStatus>(ticket.status)
  const [busy,setBusy]=useState<string|null>(null)
  const [commentOpen,setCommentOpen]=useState(false)
  const [commentText,setCommentText]=useState('')
  // Вложения (фото-отчёты) — живой режим
  ...
  // Проторолевая модель = TECHNICIAN/CLIENT/ADMIN, поэтому гейтим на CLIENT(демо)+ADMIN, а не на 'CLIENT'.
  // Точную client-company проверку делает бэкенд (вернёт 403 — покажем тостом).
  const canAccept=role==='CLIENT'||role==='ADMIN'
  const tap=(id:string)=>setDone(p=>{const n=new Set(p);n.has(id)?n.delete(id):n.add(id);return n})

  const loadAttachments=useCallback(async()=>{
    if(demo)return
    try{ setAttachments(await api.listAttachments(ticket.id)) }catch{/* список не критичен для действий */}
  },[demo,ticket.id])
  useEffect(()=>{ loadAttachments() },[loadAttachments])

  async function onPickFile(e:any){
    const file:File|undefined=e?.target?.files?.[0]
    if(e?.target) e.target.value='' // разрешить повторный выбор того же файла
    if(!file)return
    const okType=api.ATTACHMENT_ALLOWED_MIME.includes(file.type)||/\.(jpe?g|png|webp|heic|heif)$/i.test(file.name)
    if(!okType){ addToast('Только изображения: JPEG/PNG/WebP/HEIC/HEIF','error'); return }
    if(file.size>api.ATTACHMENT_MAX_BYTES){ addToast('Файл больше 25 МБ','error'); return }
  ...
  // Живое действие: вызов API → тост → рефетч статуса заявки + доски
  async function runLive(actId:string, fn:()=>Promise<unknown>, okMsg:string){
    setBusy(actId)
    try{
      await fn()
      addToast(okMsg,'success')
      try{ const fresh=await api.ticket(ticket.id); setLiveStatus(fresh.status) }catch{/* статус подтянем позже */}
      reloadBoard()
    }catch(e:any){
      addToast(e?.message||'Ошибка действия','error') // показываем реальное сообщение бэкенда
      try{ const fresh=await api.ticket(ticket.id); setLiveStatus(fresh.status) }catch{/* noop */}
    }finally{ setBusy(null) }
  }

  const Btn=({id,label,icon,danger,secondary,onAct}:{id:string;label:string;icon:string;danger?:boolean;secondary?:boolean;onAct?:()=>void})=>(
    <button disabled={busy===id} onClick={()=>{ if(onAct){onAct()} else {tap(id);if(!secondary&&!danger)addToast(label,'success')} }} className={`w-full flex items-center gap-3.5 px-5 py-4 rounded-2xl font-semibold text-[14px] transition-all active:scale-[0.97] disabled:opacity-60 ${done.has(id)?'bg-emerald-50 text-emerald-700 border-2 border-emerald-200':danger?'bg-red-50 text-red-700 border-2 border-red-200':secondary?'bg-white text-slate-700 border border-slate-200 shadow-sm':'bg-blue-600 text-white shadow-lg shadow-blue-200'}`}>
      <span className="text-[20px]">{busy===id?'⏳':(done.has(id)?'✅':icon)}</span><span>{busy===id?'Выполняется…':(done.has(id)?'Выполнено':label)}</span>
    </button>
  )
  const roleLabel={TECHNICIAN:'Техник',CLIENT:'Клиент',ADMIN:'Администратор'}[role]
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      {rejectModal&&(
        <div className="absolute inset-0 z-50 bg-black/50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-5">
            ...
            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Фото *</label>
            <div className="mt-1.5 mb-4">{!rejectModal.hasPhoto?<button onClick={()=>setRejectModal(p=>p?({...p,hasPhoto:true}):(p))} className="w-full py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-[12px] font-semibold text-slate-600">📷 Добавить фото</button>:<div className="bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2.5 flex items-center gap-2"><span className="text-emerald-600">✓</span><span className="text-[12px] font-semibold text-emerald-700">Фото добавлено</span></div>}</div>
            <button onClick={()=>{if(rejectModal.comment.length<3){setRejectModal(p=>p?({...p,err:'Комментарий обязателен'}):(p));return}if(!rejectModal.hasPhoto){setRejectModal(p=>p?({...p,err:'Фото обязательно'}):(p));return}if(demo){addToast('Заявка отправлена на доработку','info');setRejectModal(null);return}const reason=rejectModal.comment;setRejectModal(null);runLive('reject',()=>api.acceptance(ticket.id,{decision:'REJECT',comment:reason}),'Заявка отправлена на доработку')}} className={`w-full py-3.5 rounded-2xl font-bold text-[14px] ${rejectModal.comment.length>=3&&rejectModal.hasPhoto?'bg-red-600 text-white shadow-lg shadow-red-200':'bg-slate-100 text-slate-400'}`}>Отправить на доработку</button>
          </div>
        </div>
      )}
      <div className="bg-white border-b border-slate-100 px-4 py-2 flex-shrink-0 flex items-center gap-2">
        ...
        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${sBadge(st)}`}>{sLabel(st)}</span>
        {!demo&&<span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 ml-auto">live</span>}
      </div>
      <div className="flex-1 overflow-y-auto px-4 py-4 bg-slate-50 flex flex-col gap-3" style={{scrollbarWidth:'none'}}>
        {role==='TECHNICIAN'&&st==='NEW'&&<><Btn id="claim" label="Взять в работу" icon="🙋" onAct={demo?undefined:()=>runLive('claim',()=>api.claim(ticket.id),'Заявка взята в работу')}/><div className="bg-slate-50 rounded-2xl px-4 py-3 border border-slate-100 flex items-start gap-3"><span className="text-[16px]">ℹ️</span><p className="text-[12px] text-slate-500">Нажмите «Взять в работу», чтобы принять заявку.</p></div></>}
        {role==='TECHNICIAN'&&st==='ASSIGNED'&&<><Btn id="start" label="Начать работу" icon="▶️" onAct={demo?undefined:()=>runLive('start',()=>api.setStatus(ticket.id,{status:'IN_PROGRESS'}),'Работа начата')}/><div className="bg-slate-50 rounded-2xl px-4 py-3 border border-slate-100"><p className="text-[12px] text-slate-500">Подтвердите, что вы на объекте.</p></div></>}
        {role==='TECHNICIAN'&&st==='IN_PROGRESS'&&<>
          <Btn id="close" label="Отправить на приёмку" icon="✅" onAct={demo?undefined:()=>runLive('close',()=>api.setStatus(ticket.id,{status:'AWAITING_ACCEPTANCE'}),'Отправлено на приёмку')}/>
          {demo
            ? <Btn id="photo" label="Добавить фото отчёта" icon="📷" secondary/>
            : <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3">
                <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" className="hidden" onChange={onPickFile}/>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">📷 Фото-отчёт{attachments.length>0?` (${attachments.length})`:''}</p>
                  <button disabled={attBusy} onClick={()=>fileInputRef.current?.click()} className="text-[12px] font-bold text-blue-600 disabled:opacity-50 active:text-blue-700">{attBusy?'Загрузка…':'＋ Добавить фото'}</button>
                </div>
                {attachments.length===0
                  ? <p className="text-[11px] text-slate-400">Нужно ≥1 фото для отправки на приёмку.</p>
                  : <div className="flex gap-2 flex-wrap">{attachments.map(a=><img key={a.id} src={api.attachmentSrc(a.url)} alt={a.originalName} title={a.originalName} className="w-16 h-16 rounded-lg object-cover border border-slate-200 bg-slate-50"/>)}</div>}
              </div>}
          {!demo
            ? <Btn id="comment" label="Написать комментарий" icon="💬" secondary onAct={()=>setCommentOpen(v=>!v)}/>
            : <Btn id="comment" label="Написать комментарий" icon="💬" secondary/>}
          {!demo&&commentOpen&&(
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3">
              <textarea value={commentText} onChange={e=>setCommentText(e.target.value)} rows={2} placeholder="Комментарий по заявке…" className="w-full bg-slate-50 rounded-xl px-3 py-2.5 text-[13px] text-slate-800 outline-none border border-slate-200 resize-none"/>
              <button disabled={busy==='comment'||commentText.trim().length<2} onClick={()=>runLive('comment',()=>api.addComment(ticket.id,commentText.trim()),'Комментарий добавлен').then(()=>{setCommentText('');setCommentOpen(false)})} className="mt-2 w-full py-2.5 bg-blue-600 text-white rounded-xl text-[12px] font-bold disabled:opacity-50">{busy==='comment'?'Отправка…':'Отправить'}</button>
            </div>
          )}
        </>}
        ...
        {canAccept&&st==='AWAITING_ACCEPTANCE'&&<>
          ...
          {/* Реальные фото-отчёты с бэка (live) */}
          {!demo&&attachments.length>0&&<div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3"><p className="text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-2">Фото работ ({attachments.length})</p><div className="flex gap-2 flex-wrap">{attachments.map(a=><img key={a.id} src={api.attachmentSrc(a.url)} alt={a.originalName} title={a.originalName} className="w-16 h-16 rounded-lg object-cover border border-slate-200 bg-slate-50"/>)}</div></div>}
          <Btn id="accept" label="Принять работу" icon="✅" onAct={demo?undefined:()=>runLive('accept',()=>api.acceptance(ticket.id,{decision:'ACCEPT'}),'Работа принята')}/>
          {demo&&<button onClick={()=>onViewPhotos?.()} className="w-full flex items-center gap-3.5 px-5 py-4 rounded-2xl font-semibold text-[14px] bg-white text-slate-700 border border-slate-200 shadow-sm active:scale-[0.97]"><span className="text-[20px]">🖼️</span>Посмотреть фото работ</button>}
          ...
          <div className="bg-orange-50 rounded-2xl px-4 py-3 border border-orange-100 flex items-start gap-3"><span className="text-[14px]">⚠️</span><p className="text-[11px] text-orange-700">При отказе обязателен комментарий{demo?' и фото':''}.</p></div>
        </>}
        {role==='CLIENT'&&st!=='AWAITING_ACCEPTANCE'&&<div className="bg-slate-50 rounded-2xl px-4 py-6 border border-slate-100 flex flex-col items-center gap-3 text-center"><span className="text-[40px]">📋</span><p className="text-[13px] font-semibold text-slate-600">{sLabel(st)}</p><p className="text-[12px] text-slate-400">Действия доступны, когда заявка на приёмке.</p></div>}
...
// ─── Login ────────────────────────────────────────────────────────────────────
function LoginScreen({onLogin,onDemo}:{onLogin:(email:string,password:string)=>Promise<string|null>;onDemo:()=>void}) {
  ...
  const submit=async()=>{
    ...
    const e=await onLogin(email.trim(),password)
    if(e){setErr(e);setBusy(false)} // при успехе LoginScreen размонтируется
  }
  return (
    <div className="flex-1 flex flex-col bg-white px-7">
      <div className="flex-1 flex flex-col justify-center">
        <div className="flex flex-col items-center mb-10">
          <div className="w-16 h-16 rounded-2xl bg-blue-600 flex items-center justify-center mb-4 shadow-lg shadow-blue-300/40">
            <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
          </div>
          <h1 className="text-[20px] font-bold text-slate-900">ServiceManager.AI</h1>
          <p className="text-[12px] text-slate-400 mt-1">Вход в систему</p>
        </div>
        <label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Email</label>
        <input value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')submit()}} type="email" inputMode="email" autoCapitalize="none" placeholder="you@company.ru" className="w-full bg-slate-100 rounded-xl px-4 py-3 text-[14px] text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-300 mb-4"/>
        <label className="text-[12px] font-semibold text-slate-500 mb-1.5 block">Пароль</label>
        <input value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')submit()}} type="password" placeholder="••••••••" className="w-full bg-slate-100 rounded-xl px-4 py-3 text-[14px] text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-300 mb-3"/>
        {err&&<p className="text-[12px] text-red-600 mb-3 text-center">{err}</p>}
        <button onClick={submit} disabled={!canSubmit} className={`w-full rounded-xl py-3.5 text-[15px] font-bold text-white transition-colors ${canSubmit?'bg-blue-600 active:bg-blue-700':'bg-slate-300'}`}>{busy?'Вход…':'Войти'}</button>
        <button onClick={onDemo} disabled={busy} className="w-full mt-3 text-[13px] font-semibold text-blue-600 py-2 active:text-blue-700">Войти в демо-режиме</button>
      </div>
      <p className="text-[10px] text-slate-300 text-center pb-6">Mobile UX V2 · реальный вход + демо</p>
    </div>
  )
}
...
export default function App() {
  ...
  const [meInitials,setMeInitials]=useState<string|null>(null)
  const [objects,setObjects]=useState<ServiceObject[]>(OBJECTS)
  ...
  const reloadBoard=useCallback(async()=>{
    ...
    try{
      const resp=await api.board()
      const uid=api.getToken()?meId:null
      setObjects(boardToObjects(resp,{currentUserId:uid??undefined}))
    }catch(e:any){
      setBoardError(e?.message||'Не удалось загрузить заявки')
    }finally{
  ...
  const handleLogin=useCallback(async(email:string,password:string):Promise<string|null>=>{
    try{
      ...
      const disp=userDisplay(res.user)
      setMeName(disp.name);setMeInitials(disp.initials)
      setDemo(false)
      setRole((res.user?.role as UserRole)||'TECHNICIAN')
      setAuthed(true)
      // первичная загрузка доски сразу после входа
      setBoardLoading(true);setBoardError(null)
      try{
        const resp=await api.board()
        setObjects(boardToObjects(resp,{currentUserId:res.user?.id}))
      }catch(e:any){
        setBoardError(e?.message||'Не удалось загрузить заявки')
      }finally{ setBoardLoading(false) }
      return null
    }catch(e:any){
      return e?.status===401?'Неверный email или пароль':(e?.message||'Ошибка входа')
    }
  },[])

  const handleDemo=useCallback(()=>{
    api.logout()
    setDemo(true);setMeId(null);setMeName(null);setMeInitials(null);setObjects(OBJECTS);setBoardError(null)
    setAuthed(true)
  },[])

  const handleLogout=useCallback(()=>{
    api.logout()
    setAuthed(false);setDemo(true);setMeId(null);setMeName(null);setMeInitials(null);setObjects(OBJECTS);setBoardError(null)
    setScreens([{kind:'main',navTab:'home'}])
  },[])
  ...
  const showBottomNav=current.kind==='main'&&authed

  function renderScreen() {
    if(!authed) return <LoginScreen onLogin={handleLogin} onDemo={handleDemo}/>
    ...
    if(current.kind==='awaiting') return demo?<AwaitingScreen onOpenTicket={t=>push({kind:'workspace',ticket:t})} push={push} addToast={addToast}/>:<LiveAwaitingScreen onBack={pop} onOpenChat={(id,title)=>push({kind:'chat-detail',ticketId:id,ticketTitle:title})} addToast={addToast}/>
    if(current.kind==='profile') {
      const onRole=(r:string)=>{setRole(r as UserRole);addToast(`Роль изменена: ${({TECHNICIAN:'Техник',CLIENT:'Клиент',ADMIN:'Администратор'} as Record<string,string>)[r]||r}`,'success')}
      const onCont=(c:ContourMode)=>{setContour(c);if(c==='management')addToast('Переключение в управленческий контур','info')}
      return demo
        ? <ProfileScreen push={push} role={role} onRoleChange={onRole as (r:UserRole)=>void} contour={contour} onContourChange={onCont}/>
        : <LiveProfileScreen role={role} onRoleChange={onRole} contour={contour} onContourChange={onCont} onOpenCompany={()=>push({kind:'company'})} onOpenSettings={()=>push({kind:'settings'})} onBack={pop} onLogout={handleLogout}/>
    }
    if(current.kind==='company') return demo?<CompanyScreen onBack={pop}/>:<LiveCompanyScreen onBack={pop} objects={objects}/>
    if(current.kind==='settings') return demo?<SettingsScreen onBack={pop}/>:<LiveSettingsScreen onBack={pop}/>
    ...
    if(current.kind==='equipment-list') return <EquipmentListScreen locationId={current.locationId} locationName={current.locationName} onBack={pop} onOpenDetail={e=>push({kind:'equipment-item',equipment:e})}/>
    if(current.kind==='equipment-item') return <LiveEquipmentDetailScreen equipment={current.equipment} onBack={pop}/>
    if(current.kind==='chat-detail') return <ChatDetailScreen ticketId={current.ticketId} ticketTitle={current.ticketTitle} currentUserId={meId} onBack={pop}/>
  ...
  return (
    <LiveContext.Provider value={{demo,meId,meName,meInitials,objects,boardLoading,boardError,reloadBoard}}>
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-[#0f1f4a] to-blue-950 flex flex-col items-center justify-center p-6 gap-4">
              ...
              <div className="flex-shrink-0 bg-white border-t border-slate-100">
                <div className="flex items-center justify-around px-1 pt-2 pb-5">
                  {navDefs.map(item=>{
                    ...
                    // Роль CLIENT: 2-й таб «Приёмка» вместо «Обходы»
                    if(item.id==='patrols'&&role==='CLIENT') return <button key="awaiting" onClick={()=>push({kind:'awaiting'})} className="flex flex-col items-center gap-1 min-w-[52px] py-0.5 active:scale-95 transition-transform"><svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg><span className="text-[10px] font-semibold text-slate-400">Приёмка</span></button>
      ...
      {import.meta.env.DEV&&<p className="text-[10px] text-slate-600 text-center max-w-[360px] leading-relaxed">
