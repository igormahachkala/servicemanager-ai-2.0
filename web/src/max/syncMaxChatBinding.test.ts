...
describe('syncMaxChatBinding', () => {
  ...
  it('skips ordinary browser login without MAX initData', async () => {
    getToken.mockReturnValue('jwt')
    isImpersonating.mockReturnValue(false)
    const { getMaxEnvironmentContext, getWebApp } = await import('./maxBridge')
  ...
  it('loads the MAX bridge on /m when initData is not already in memory', async () => {
    getToken.mockReturnValue('jwt')
    isImpersonating.mockReturnValue(false)
    getMaxBinding.mockResolvedValue({ binding: null })
    createMaxBinding.mockResolvedValue({ created: true, binding: { status: 'ACTIVE' } })
    const { getMaxEnvironmentContext, getWebApp, loadMaxBridgeScript } = await import('./maxBridge')
    vi.mocked(getMaxEnvironmentContext)
      .mockReturnValueOnce({ initData: '', detected: false })
      .mockReturnValueOnce({ initData: 'from-bridge', detected: true })
    vi.mocked(getWebApp).mockReturnValue(null)
    expect(await syncMaxChatBinding()).toBe('bound')
    expect(loadMaxBridgeScript).toHaveBeenCalled()
    expect(createMaxBinding).toHaveBeenCalledWith('from-bridge')
  })
})
