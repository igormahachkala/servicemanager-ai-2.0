...
<separate_calls>
Проверка, постановка замка и слияние PR выполняются разными вызовами.
Каждый следующий начинается после того, как предыдущий дал результат
и результат разобран.

Склеенные в одну цепочку, они выполняются подряд: отказ постановки —
already exists, код 128 — не мешает слиянию, потому что слияние отдельная
команда и ничто её не останавливает. Проверено на живом случае 2 сентября
2026: контур был занят чужим замком, постановка отказала, слияние прошло.

Это относится к любому месту, где предусловие предшествует необратимому
действию, а не только к замку.
</separate_calls>
