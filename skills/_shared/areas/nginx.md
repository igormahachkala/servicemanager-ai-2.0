...
<area name="nginx">
<what>
docs/nginx-*.conf — копии vhost nginx. configs/*.conf — фрагменты для
вставки в /etc/nginx/nginx.conf (log_format, brotli, realip), не целые
файлы конфигурации. Работающая конфигурация лежит в системном каталоге
сервера, nginx работает вне compose. Развёртывание контейнеров её не
затрагивает, пересборка не применяет.
</what>
<stop>
...
<if name="фрагменты configs/">
Ветка только кладёт configs/*.conf — критерий в репозитории контура:
файлы на месте после git pull. Live nginx сам не меняется. B-2 и B-3
...
<snippets>
Файлы configs/*.conf нельзя копировать поверх /etc/nginx/nginx.conf:
это обрывки блока http, не полный файл. Оператор вставляет содержимое
в указанное место. Фрагмент realip (B-7) не применять до смены DNS
на Curator.
</snippets>
...
<f>sudo cp configs/*.conf /etc/nginx/nginx.conf — затрёт рабочий nginx.conf.</f>
</forbidden>
</area>
