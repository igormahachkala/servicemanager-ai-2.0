...
Код 0 — контур свободен, идти дальше. Ненулевой код останавливает:
проверка обязана давать код, а не вывод на прочтение, блок why_test_z
шага 7. Имя замка фиксированное,
...
<why_test_z>
Проверка обязана давать код возврата, а не вывод на прочтение. Голый
ls-remote возвращает 0 и когда контур свободен, и когда занят: разница
только в напечатанных строках, и цепочка команд идёт дальше независимо
от того, что там напечатано. test -z даёт ненулевой код при занятом
контуре и останавливает цепочку сам.
То же соображение записано в блоке why_separate шага 8 про git status.
</why_test_z>
<constraint name="проверка и слияние — разные вызовы">
Не объединять в одну цепочку команд проверку замка, постановку и слияние.
Каждый следующий шаг выполняется после того, как предыдущий дал результат
и этот результат разобран. Склеенные в один вызов, они выполняются подряд:
отказ постановки замка не мешает слиянию, потому что слияние — отдельная
команда, и ничто её не останавливает.
Проверено на живом случае 2 сентября 2026: постановка отказала
с already exists, слияние в занятый контур всё равно прошло.
</constraint>
<on_precondition_failure>
...
<why_fetch_first>
Без неё постановка падает при свободном контуре: fatal: tag 'prod-busy' already
exists, код 128, ещё до push. Локальная копия чужого замка появляется
по нашему же порядку — при занятом контуре предписано
git fetch origin tag prod-busy, чтобы прочитать, чей замок; владелец потом
...
и необратимо: после него код в prod, даже если развёртывание не состоится.
Для областей, где шаг 5 пропускается — none, scripts, skills, nginx, —
слияние вообще единственное необратимое действие круга, и без этого вопроса
оно проходило бы без подтверждения вовсе.
</why>
<constraint>
