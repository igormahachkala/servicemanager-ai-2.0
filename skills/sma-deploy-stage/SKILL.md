...
<on_precondition_failure>
Stage занят другим агентом. Не сливать, не разворачивать. Прочитать тело
замка блоком read общего файла и показать пользователю: чья ветка, какой PR,
с какого времени. Чужой замок не снимать, блок foreign_lock.
</on_precondition_failure>
<acquire>
...
Замок stage-busy не снимать: контур остаётся за вами до приёмки.
Повторный проход шага 7 увидит собственный замок — сверить имя ветки
в теле, своё продолжать, чужое разбирать по foreign_lock. Вопрос открыт,
В-01 реестра: снимать ли замок при отказе приёмки.
...
<on_failure>
Не сбрасывать. Показать пользователю имя замка и тело — чья ветка, какой PR,
с какого времени. Чужой замок не снимать, блок foreign_lock общего файла.
</on_failure>
</precondition>
