...
Файл длиннее 300 строк, ниже карта разделов.
  ...
  part E  проверка обнаружения скилов
...
<part id="E" name="проверка обнаружения скилов">
<when>
Один раз на машину после клонирования репозитория и один раз на каждого
нового агента. Дальше — при жалобе «скилл не сработал».
</when>
<why>
Тексты скилов лежат в skills/&lt;имя&gt;/SKILL.md, а обнаруживаются через
символьные ссылки в .claude/skills/. Ссылку в клоне может подменить обычный
файл: при core.symlinks=false и на Windows без Developer Mode git кладёт
текстовый файл со строкой пути внутри. SKILL.md по такому пути не находится,
скилы не подхватываются, и отказ выглядит как их отсутствие в репозитории.
</why>

<step id="1" name="проверить ссылки и фронтматтер">
<run script="skills/_shared/scripts/skills-check.sh">
skills/_shared/scripts/skills-check.sh
</run>
...
Код 3 — каталога обнаружения нет либо он пуст. Проверить, что клонирован
нужный репозиторий и рабочая копия не обрезана.
</on_failure>
<constraint>
Скрипт читает файлы и ничего не меняет. Починку выполняет агент отдельной
командой после разбора вывода.
</constraint>
</step>

<step id="2" name="проверить срабатывание у агента">
<why>
Шаг 1 доказывает, что файлы на месте и пригодны. Он не доказывает, что
конкретный агент их подхватывает: список скилов собирает среда, а не
репозиторий. Это проверяется только запуском.
</why>
<action>
Открыть чистую сессию агента в корне репозитория и задать задачу
естественной формулировкой, без имени файла и без слова «скилл»:
«проверь, как в этом репозитории обнаруживаются скилы и видит ли их агент».
</action>
<expect>
Агент сам выбирает sma-skill-probe и возвращает три строки: маркер
срабатывания SKILL-PROBE-OK, путь файла, из которого прочитаны инструкции,
и маркер внешнего файла EXTERNAL-READ-OK.
</expect>
<on_failure>
Скилл не выбран — среда каталог обнаружения не читает либо описание
не совпало с формулировкой. Проверить, что агент вообще видит список скилов:
вызвать sma-skill-probe по имени. Сработал по имени, но не по формулировке —
дело в description, а не в раскладке.
Первый маркер получен, второй нет — агент не переходит по ссылке за пределы
каталога скила. Общие файлы _shared в такой среде не читаются, и все скилы,
которые на них ссылаются, работают неполно. Остановиться и сообщить
пользователю.
</on_failure>
<record>
Результат прогона записывать в skills/_logs/probe-log.md с датой, средой
и версией: подтверждение относится к конкретной среде, а не к репозиторию
вообще.

Строка журнала: дата, среда (агент и версия, машина), вершина origin/prod
на момент прогона, исход — оба маркера получены либо какой не получен.

Файл лежит в репозитории намеренно: записи нужны другим агентам и переживают
переустановку машины. Каталог _claude для этого не годится, он в .gitignore
и виден только на той машине, где создан.
Решено 2 сентября 2026, В-03 реестра.
</record>
<area_note>
Правка probe-log.md попадает в область skills карты area-map.sh:
автоматических проверок нет, развёртывание не требуется, но доставка идёт
обычным потоком через ветку и PR.
</area_note>
</step>
</part>
