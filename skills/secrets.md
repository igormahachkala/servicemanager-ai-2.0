...
для skills/deploy-stage.md и skills/deploy-prod.md. Читать до первой команды,
...
<rationale>
Ограничение прав защитой не является: deploy состоит в группе docker, а это
равносильно root — любой участник монтирует произвольный каталог хоста
в контейнер и читает что угодно. Граница только поведенческая.

Всё напечатанное попадает в переписку и в логи. Секрет, показанный один раз,
считается раскрытым независимо от того, кто это читал.
</rationale>
