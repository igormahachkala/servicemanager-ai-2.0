...
describe('TicketsAcceptanceService.decide', () => {
  ...
  it('allows a contractor MASTER through current PRIMARY linked-client access', async () => {
    const master = { id: 'master-1', role: UserRole.MASTER, companyId: PROVIDER_ID }
    const { svc, serviceContracts } = makeSetup({
      actorRole: UserRole.MASTER,
      actorCompanyType: CompanyType.PROVIDER,
    ...
    expect(serviceContracts.getLinkedClientAccess).toHaveBeenCalledWith(PROVIDER_ID, CLIENT_ID)
  })

  it('allows a contractor ADMIN through current SECONDARY linked-client access', async () => {
    const admin = { id: 'admin-provider-1', role: UserRole.ADMIN, companyId: PROVIDER_ID }
    const { svc, serviceContracts } = makeSetup({
      actorRole: UserRole.ADMIN,
      actorCompanyType: CompanyType.PROVIDER,
