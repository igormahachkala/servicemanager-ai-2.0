...
describe('105B конвейер уведомлений', () => {
  it('запись Notification остаётся единственной точкой', () => {
    ...
    const batchWrites = SERVICE.match(/prisma\.notification\.createMany\(/g) ?? [];
  ...
  it('ворота стоят до записи, а не после неё', () => {
    ...
    const body = SERVICE.slice(at, at + 1400);
    expect(body.indexOf('preferenceGate')).toBeLessThan(body.indexOf('prisma.notification.create('));
  });

  it('канал PUSH проходит через те же ворота', () => {
    const at = SERVICE.indexOf('private async pushTicketEvent(');
    expect(at).toBeGreaterThan(-1);
    const body = SERVICE.slice(at, at + 2000);
    expect(body).toContain('preferenceGate');
    expect(body).toContain('NotificationChannel.PUSH');
    expect(body.indexOf('preferenceGate')).toBeLessThan(body.indexOf('this.push.sendToUser('));
  });
  ...
  it('ворота не умеют добавлять получателей: единственный выход — подмножество входа', () => {
    const gate = readFileSync(join(__dirname, 'notification-preference-gate.ts'), 'utf8');
