 ...
 */
const SUPPRESSING_SOURCES = new Set(['USER_OVERRIDE', 'COMPANY_ROLE_OVERRIDE']);

export function shouldSuppress(decision: NotificationPreferenceDecision): boolean {
...
export class NotificationPreferenceGate {
  ...
  /** Решение по одному получателю: для канала PUSH, где строки нет. */
  async allows(row: PreferenceGateRow, channel: NotificationChannel): Promise<boolean> {
    const [kept] = await this.filterRows([row], channel);
    return !!kept;
  }

  private async anyPreferenceConfigured(companyIds: string[]): Promise<boolean> {
    if (!companyIds.length) return false;
    const [userScoped, companyScoped] = await Promise.all([
      this.prisma.userNotificationPreference.count({
        where: { companyId: { in: companyIds } },
        take: 1,
      } as any),
      this.prisma.companyNotificationRolePreference.count({
        where: { companyId: { in: companyIds } },
        take: 1,
      } as any),
    ]);
    return userScoped > 0 || companyScoped > 0;
  }

  private async loadRoles(rows: PreferenceGateRow[]): Promise<Map<string, UserRole>> {
