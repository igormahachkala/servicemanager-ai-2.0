...
describe('106B correction authorization is the canonical management grant', () => {
  ...
  it.each(['createShiftCorrection', 'getShift'] as const)(
    'keeps the 111A management-surface gate on %s',
    (method) => {
      const guards = Reflect.getMetadata(GUARDS_METADATA, WorkforceController.prototype[method])
      expect(guards).toContain(ManagementSurfaceGuard)
    },
  )
})
